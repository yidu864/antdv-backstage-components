...
export default {
  data() {
    return {
      ...
      localLoading: { spinning: false, indicator: <a-icon type="loading" /> },
      localDataSource: [],
      localPagination: Object.assign({}, this.pagination, { showTotal: total => `共 ${total} 条` })
    }
  },
    ...
    'localPagination.current'(val) {
      ...
      this.pageURI &&
        this.$router.push({
          ...
          params: Object.assign({}, this.$route && this.$route.params, {
            pageNo: val
          })
        })
    },
  ...
  created() {
    const { pageNo } = (this.$route && this.$route.params) || 1
  ...
  methods: {
    ...
    loadData(pagination, filters, sorter) {
      this.localLoading.spinning = true
      ...
      if (
        ...
        result.then(r => {
          this.localPagination =
            (this.showPagination &&
              Object.assign({}, this.localPagination, {
                current: r.paging.pageIndex, // 返回结果中的当前分页数
                total: r.paging.totalRecord, // 返回结果中的总记录数
                showSizeChanger: this.showSizeChanger,
          ...
          try {
            if (
              ['auto', true].includes(this.showPagination) &&
              r.paging.totalRecord <= r.paging.pageIndex * this.localPagination.pageSize
            ) {
          ...
          this.localLoading.spinning = false
        })
      }
    },
